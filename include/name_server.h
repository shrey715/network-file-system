...
typedef struct {
  ...
  char owner[MAX_USERNAME];
  int ss_id;
  int version; // File version for Staleness Detection
  time_t created_time;
...
typedef struct {
  ...
  int replica_id;     // ID of the backup server
  int replica_active; // Is the backup currently active?
  char **files;
  int file_count;
} StorageServerInfo;
...
typedef struct {
  ...
  AccessRequest access_requests[MAX_FILES]; // Max one request per file per user
                                            // (simplified)
  int request_count;
...
int nm_register_file(const char *filename, const char *folder_path,
                     const char *owner, int ss_id);
FileMetadata *nm_find_file(const char *filename);
FileMetadata *nm_find_file_in_folder(const char *filename,
                                     const char *folder_path);
int nm_delete_file(const char *filename);
int nm_check_permission(const char *filename, const char *username,
                        int need_write);
...
int nm_check_folder_permission(const char *foldername, const char *username,
                               int need_write);
int nm_list_folder_contents(const char *foldername, const char *username,
                            char *buffer, size_t buffer_size);

// Storage server operations
int nm_register_storage_server(int server_id, const char *ip, int nm_port,
                               int client_port);
StorageServerInfo *nm_find_storage_server(int ss_id);
int nm_select_storage_server(void);

// Access control
int nm_add_access(const char *filename, const char *username, int read,
                  int write);
int nm_remove_access(const char *filename, const char *username);
int nm_add_folder_access(const char *foldername, const char *username, int read,
                         int write);

// Access requests
int nm_request_access(const char *filename, const char *requester,
                      int read_requested, int write_requested);
int nm_view_requests(const char *filename, const char *owner, char *buffer,
                     size_t buffer_size);
int nm_approve_request(const char *filename, const char *owner,
                       const char *requester);
int nm_deny_request(const char *filename, const char *owner,
                    const char *requester);
